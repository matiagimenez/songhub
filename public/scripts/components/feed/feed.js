...
window.addEventListener("scroll", () => {
  ...
  async function fetchPosts() {
    ...
    if (window.location.href.includes("user")) {
      ...
      endpoint = `/post/profile?page=${currentPage + 1}&username=${username}`;
    } else {
    ...
    try {
      ...
      data.map((post) => {
        ...
        const postElement = ElementBuilder.createElement("article", "", {
        ...
        postElement.innerHTML = `
					 <figure>
                        <section class="article-img-container">
                            <img loading="lazy" class="image-border" width="100px" height="100px" src="${
                              post.COVER_ID
                            }" alt="Portada del álbum '${post.TITLE}' ${
          post.NAME
        }" />
                        </section>
                        <section class="user-info">
                            <img loading="lazy" class="user-img" height="25px" width="25px" src="${
                              post.USER.fields.SPOTIFY_AVATAR
                            }" alt="Imagen de perfil de '{{ post.USER.fields.USERNAME }}'" />
                            <p class="user-name">
                            	<a href="/user?username=${
                                post.USER.fields.USERNAME
                              }">${post.USER.fields.NAME}</a>
                                <span>$${post.USER.fields.USERNAME}</span>
                            </p>
                            <p class="post-time">${post.TIME_AGO}</p>
                        </section>
                        <figcaption>
                            <h3 class="song-title" id="post-${
                              post.POST_ID
                            }-song-title">
                                <a href="/content?id=${
                                  post.CONTENT_ID
                                }&type=${type}">${post.TITLE}</a>
                            </h3>
                            <h4 class="artist-title" id="post-${
                              post.POST_ID
                            }-artist-title">${post.NAME}</h4>
                            ${tagElements}
                            <p class="stars">
                                ${starElements}
                            </p>
                    ...
                    <footer>
                        ...
                        <section class="reaction-container like-container">
                            <button class="heart-button" post-id=${
                              post.POST_ID
                            }>
																<i class=${
                                  post.LIKED ? " ph-fill active " : " ph "
                                } ph-heart icon heart-icon"></i>
